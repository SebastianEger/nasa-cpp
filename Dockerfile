...
RUN cmake -Bbuild -GNinja -DCMAKE_INSTALL_PREFIX=/install -H. -DPAHO_WITH_MQTT_C=ON \
...
RUN cmake -Bbuild -GNinja -DCMAKE_INSTALL_PREFIX=/install -DBUILD_SHARED_LIBS=${BUILD_SHARED}
RUN cmake --build build/ --target install
...
RUN cmake -Bbuild -GNinja -DCMAKE_INSTALL_PREFIX=/install -DJSON_BuildTests=OFF -DBUILD_SHARED_LIBS=${BUILD_SHARED}
RUN cmake --build build/ --target install
